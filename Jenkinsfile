pipeline {
  ...
  stages {
    ...
    stage('Checkout') {
      steps {
        deleteDir()
        // укажи здесь нужную ветку, если не lab7
    ...
    stage('Provision tools (Chrome + matching chromedriver)') {
      steps {
        sh '''
          ...
          # Подбор совместимого chromedriver по Chrome for Testing
    ...
    stage('Start QEMU (OpenBMC)') {
      steps {
        sh '''
          ...
          # проверим, что порты свободны (через nc)
          ...
          # ожидание: SSH (до 180с), затем HTTPS (до 900с)
          wait_tcp() {
